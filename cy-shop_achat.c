...
// Validation de prénom & nom conventionnel
...
// Validation de genre conventionnel
...
int login() {
  ...
  printf(
      "A PRESENT VEUILLEZ VOUS CONNECTER EN SAISISSANT VOS IDENTIFIANTS\n\n");
...
// Validation de la taille du produit
...
// Validation de la couleur du produit
...
// retrouver un produit par sa taille
...
// retrouver un produit par sa couleur
...
// retrouver un produit par son nom
...
// retrouver un produit par sa référene
...
// afficher produits en rupture de stocks
...
// afficher la liste des 5 produits avec le plus faible stock
...
// calculer l'espace de stockage de produits
...
void addProduct() {
  ...
  while (newProduct.reference <= 0) {
    printf("CETTE REFERENCE N'EST PAS VALABLE. VEUILLEZ SAISIR UNE REFERENCE "
           "VALABLE : \n");
    scanf("%d", &newProduct.reference);
  }
...
// ajouter un produit en remplissant ses caractéristiques
  ...
  do {
    ...
    case 2: {
      ...
      if (index != -1) {
        ...
        printf(
            "AUCUN MODELE AVEC LE NUMERO DE REFERENCE '%d' N'A ETE TROUVE.\n",
            productReference);
      }
      printf("\n");
      break;
...
// afficher le stock d'un modèle en le recherchant par son nom ou sa référence
void increaseProductStock() {
  ...
  do {
    ...
    case 1: {
      ...
      if (index != -1) {
        int quantity;
        printf("VEUILLEZ SAISIR LA QUANTITE DE VEHICULES DE CE MODELE A "
               "RAJOUTER : ");
        scanf("%d", &quantity);
        printf("\n");
        products[index].quantity += quantity;
        printf("LE STOCK DU MODELE '%s' A BIEN ETE MIS A JOUR. VOICI LE STOCK "
               "DE VEHICULES DE CE MODELE : %d\n",
    ...
    case 2: {
      ...
      if (index != -1) {
        int quantity;
        printf("VEUILLEZ SAISIR LA QUANTITE DE VEHICULES DE CE NUMERO DE "
               "REFERENCE A RAJOUTER : ");
        scanf("%d", &quantity);
        products[index].quantity += quantity;
        printf(
            "LE STOCK DU MODELE QUI A POUR NUMERO DE REFRENCE '%d' A BIEN ETE "
            "MIS A JOUR. VOICI LE STOCK DE VEHICULES DE CE MODELE : %d\n",
            productReference, products[index].quantity);
      } else {
        printf(
            "AUCUN MODELE AVEC LE NUMERO DE REFERENCE '%d' N'A ETE TROUVE.\n",
...
// augmenter le stock d'un produit en le recherchant par son nom ou sa référence
void manageStock() {
  int choice;
...
// retrouver un client par son pseudo
...
// afficher l'historique d'achats du client
void purchaseProduct(Customer *customer) {
  ...
  if (indexBySize != -1 && indexByColor != -1 && indexBySize == indexByColor &&
    ...
    printf("LE MODELE '%s' DE TAILLE '%s' DE COULEUR '%s' AVEC LA REFERENCE "
           "'%d' N'EST PAS DISPONIBLE DANS NOTRE CATALOGUE.\n",
           productName, productSize, productColor, productReference);
  }
}
// acheter un produit en sélectionnant les caractéristiques de ce dernier
...
// mot de passe pour rentrer dans le MODE GESTION
